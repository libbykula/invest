...
def get_dt_effect(GA_i):
    dt_effect = -uhi_max*(3/(5*(np.exp(15/2 - (3*GA_i)/2) + 1)) + 1/5) # So I want to apply this to each pixel of green_area raster! 
    return dt_effect
...
def partial_temp_partial_lc(dt_effect_path,
                             dt_effect_kc_path,
                             binary_raster_path,
                             out_path=os.path.join(intermediate_dir, "partial_temp_partial_lc_v02.tif"),
    ...
    binary_raster_ds = gdal.Open(binary_raster_path, gdal.GA_ReadOnly)
    binary_raster = binary_raster_ds.GetRasterBand(1).ReadAsArray().astype(float)
    ...
    change_shade_raster = binary_raster*change_shade
    change_albedo_raster = binary_raster*change_albedo
    change_kc_raster = binary_raster*change_kc
    
    # --- Compute weighted sum ---
    result = dt_effect * 3 * change_shade_raster + dt_effect * change_albedo_raster + dt_effect_kc * change_kc_raster
    result[mask] = nodata
...
partial_temp_partial_lc(
    ...
    binary_raster_path = os.path.join(output_dir, 'urban_watch_binary_7.tif')
)
