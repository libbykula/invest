...
# 1. Convolve building area raster with the mixing kernel once
d_air_mix_pixels = 600
building_area_raster_path = os.path.join(output_dir, 'urban_watch_binary_0.tif')
building_area_convolved_path = os.path.join(output_dir, 'building_area_convolved.tif')

# convolve_2d_by_exponential(
#     decay_kernel_distance=d_air_mix_pixels,
#     signal_raster_path=building_area_raster_path,
#     target_convolve_raster_path=building_area_convolved_path)
# building_area_convolved[i] = sum_j A(j) * K(j-i)
...
biophysical_table_changes = biophysical_table_changes.reset_index() 
...
# partial_path = os.path.join(intermediate_dir, "partial_temp_partial_lc_v03.tif")
# Making it more general to switch from any lucode to tree! 
def partial_temp_partial_lc(dt_effect_path,
                             dt_effect_kc_path,
                             building_area_convolved_path,
                             out_path,
                             change_shade, 
                             change_albedo,
                             change_kc,
    ...
    # --- Read building area convolved raster ---
    building_area_convolved_ds = gdal.Open(building_area_convolved_path, gdal.GA_ReadOnly)
    building_area_convolved = building_area_convolved_ds.GetRasterBand(1).ReadAsArray().astype(float)
    ...
    result = dt_effect * 3 * change_shade + dt_effect * change_albedo + dt_effect_kc * change_kc * building_area_convolved
...
for i in range(0,9):
    shade = biophysical_table_changes.loc[biophysical_table_changes['index']==i, 'shade'].iloc[0] 
    albedo = biophysical_table_changes.loc[biophysical_table_changes['index']==i, 'albedo'].iloc[0] 
    kc = biophysical_table_changes.loc[biophysical_table_changes['index']==i, 'kc'].iloc[0] 

    partial_temp_partial_lc(dt_effect_path = os.path.join(intermediate_dir,
                                                          'dt_effect_unweighted_convolved_buff.tif'),
                            dt_effect_kc_path = os.path.join(intermediate_dir, 
                                                             'dt_effect_kc_buff_convolved.tif'),
                            building_area_convolved_path = building_area_convolved_path,
                            out_path = os.path.join(output_dir, 
                                                  'partial_temp_partial_lc_' + str(i) + '_to_tree.tif'),
                            change_shade=shade,
                            change_albedo=albedo,
                            change_kc=kc
                            )
...
def creating_ranking_raster(partial_cdd_partial_t_raster_path,
    ...
    # --- Pixelwise multiply ---
    result = arr1 * arr2 * price_const * mc_const * floors
...
#                         partial_t_partial_lc_raster_path=partial_path,
#                         out_path = os.path.join(output_dir, "ranking_raster_dollars_v03.tif"))
...
# ranking_array_path = os.path.join(output_dir, "ranking_raster_dollars_v03.tif")
# barren_path = os.path.join(output_dir, 'urban_watch_binary_7.tif')

# pygeoprocessing.raster_calculator([(ranking_array_path, 1), (barren_path, 1)],
#             modify_classification,
#             os.path.join(output_dir, 'ranking_dollars_barren_only_v03.tif'),
#             gdal.GDT_Float32,
#             nodata_target=999.0)

#### Ok, now I need to do the actual optimization!!! 
# So, how many pixels should change at once, and do they need to be contiguous, or just within a given radius of each other??? try 100 pixels at a time? 1000?

# I think I need to turn it into a ranking first but IDK? Maybe not, just sort it... 
# Get them in Census tracts??? which tract to replant to first???

# ranking_raster_path = os.path.join(output_dir, 'ranking_dollars_barren_only_v03.tif')

# array, nodata, geotransform, projection = select_replant_pixels.read_raster(ranking_raster_path)
...
# # Unconstrained 
# selected, total = select_replant_pixels.select_top_n_unconstrained(array, 10000, mask=mask)
# print('unconstrained mode: ', f"{int(selected.sum())} pixels, total savings = {total:,.2f}")

# select_replant_pixels.write_mask_raster(os.path.join(output_dir, 'ranking_dollars_optimal_uncons.tif'), selected, geotransform, projection)

# # Radius 
# selected, total, center = select_replant_pixels.select_best_within_radius(array, 10000, radius = 100, mask=mask)
# print('radius mode: ', f"{int(selected.sum())} pixels, total savings = {total:,.2f}")

# select_replant_pixels.write_mask_raster(os.path.join(output_dir, 'ranking_dollars_optimal_radius100.tif'), selected, geotransform, projection)

# # Connected
# selected, total = select_replant_pixels.select_best_connected_region(array, 10000, mask=mask, connectivity=4)
# print('connected mode: ', f"{int(selected.sum())} pixels, total savings = {total:,.2f}")

# select_replant_pixels.write_mask_raster(os.path.join(output_dir, 'ranking_dollars_optimal_connected.tif'), selected, geotransform, projection)
